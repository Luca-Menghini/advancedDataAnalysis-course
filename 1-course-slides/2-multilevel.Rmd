...
**6. Absence of collinearity**: no linear relationship between different predictors \newline

\fontsize{7.5pt}{12}\selectfont
Additional LMER assumptions \newline
In LMER, **assumptions 1-4** also apply to 'cluster-level residuals' (i.e., random effects).

\fontsize{6.5pt}{12}\selectfont \color{violet}
**7. Linearity, Normality, Homoscedasticity, & Independence of random effects**: \color{black} Random intercept $\lambda_{0j}$ and random slope $\lambda_{1j}$ should be normally distributed with $\lambda_{0j} \sim \mathcal{N}(0,\,\tau_{00}^{2})$ and $\lambda_{1j} \sim \mathcal{N}(0,\,\tau_{11}^{2})$, their variance should be homogeneous across the levels of $x$ variables, and they should be independent from predictors
...
__Homoscedasticity & independence__: *no* trends in residuals or their variance over $x$ `r fontawesome::fa(name = "face-smile",fill="green", height = "0.8em")`
```{r out.width="150px",fig.width=4,fig.height=2,eval=FALSE}
...
plot(residuals(m4) ~ insa$stress.cmc,pch=20,col="gray",main="Residuals by stress.cmc")
abline(lm(residuals(m4) ~ insa$stress.cmc),col="red",lty=2,lwd=2)
plot(residuals(m4) ~ insa$insomnia,xlab="",main="Residuals by sex")
```
...
__Homoscedasticity & independence__: No marked trends in random effects or their variance `r fontawesome::fa(name = "face-smile",fill="green", height = "0.8em")`, but slightly higher RI in \newline insomnia than in controls  `r fontawesome::fa(name = "face-frown",fill="orange", height = "0.8em")`
```{r out.width="150px",fig.width=4,fig.height=2,eval=FALSE}
...
\fontsize{7pt}{12}\selectfont 
Whether using LM or LMER, we need to \color{violet} *avoid using too correlated predictors* (**multicollinearity**)\color{black}, otherwise they 'steal' explained variance to each other.

The **variance inflation factors** (VIF) tell us ...
```{r out.width="150px",fig.width=6,fig.height=3,echo=FALSE}
car::vif(m4)
barplot(car::vif(m4),ylim=c(0,10),main="VIF")
abline(h = 5, col="red")
```

**Influential cases** are data points that substantially change (*influence*) one or more parameter estimates (*multivariate outliers*).

```{r out.width="150px",fig.width=6,fig.height=3,echo=FALSE}
car::vif(m4)
barplot(vif(m4),ylim=c(0,10),main="VIF")
abline(h = 5, col="red")
```
...
Fixed effects coefficient interpretation: we saw that the coefficient estimate and its standard error (SE) are used to compute *t*-values and 95% confidence intervals (CI) (see slide #X). 

p values, corretions

microscope: undetermined degrees of freedom

## Multilevel model comparison

Another way to quantify the importance of a predictor is by considering ...
