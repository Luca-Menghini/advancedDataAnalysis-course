...
```
For instance, functions that compute **descriptive statistics**:
```{r }
...
To **select 1+ values** from a matrix, we use again **squared brackets**, but this time we use the syntax \color{blue} `matrix_name[row_number, col_number]` \color{black}
```{r }
...
\fontsize{7.5pt}{12}\selectfont
There are 5 students (Gina, Pina, Tina, Maria, and Lina), each with a registration number (507, 535, 566, 955, 515) and a score at the exam (19, 27, 28, 25, 25). \newline \fontsize{5.5pt}{12}\selectfont Source: Pastore, M. (2021). Master's degree course on "Data analysis in community psychology". \newline \fontsize{7.5pt}{12}\selectfont

1. Create 3 *vectors* including, respectively, students' name, serial numbers, and exam score

2. Join the 3 vectors to create a *dataframe*

3. Delete the 3 original *vectors* from the workspace \newline (only the dataframe should remain)

4. Select and print Maria's grade

5. Select the row including all the values belonging to Pina

6. Select all students with exam score equal to 25 (you can also use the \color{blue} `subset()` \color{black} function - see \color{blue} `?subset`\color{black})

7. Select all students with an exam score higher than 20 () (you can also use the \color{blue} `subset()`\color{black})

8. Compute the mean and standard deviation of the exam scores
...
\fontsize{7pt}{12}\selectfont
The first step in any data analysis with R is to read a dataset and save it in the workspace. To do so, we need to use a specific function based on the data format \newline (e.g., CSV, xlsx, txt, sav). The default R data format is RData.
```{r eval=FALSE}
# file .RData
...
- \color{blue} Open or create a file `.xlsx` on your PC, save it in a `.csv` (comma separated values) format and read it in R
- \color{blue} Now try to directly read the `.xlsx` file (if you have doubts, just Google "*how to read xlsx file with R*" `r fontawesome::fa(name = "face-smile", fill="#3333B2",height = "1em")`)
- \color{blue} In both cases, note the class and the structure of the imported object

## Hands on `r fontawesome::fa(name = "r-project", fill="#3333B2",height = "1em")`: Course questionnaire

\fontsize{6.5pt}{12}\selectfont

1. Download the files `studqs.RData` and `studqs.csv` from \color{blue} [Github](https://github.com/Luca-Menghini/advancedDataAnalysis-course/) \color{black} (`4-data` folder; select `File` > `Raw` > `Download` or right click > `Save as`) or \color{blue} [Moodle](https://psico.elearning.unipd.it/enrol/index.php?id=4700#section-0) \color{black} ("Data" folder), save the file in a folder and set that folder as the working directory.

3. Read both files with R: what are their class? What is the class of the variables included in each of them?

4. Use the \color{blue} `describe()` \color{black} function from the \color{blue} `psych` \color{black} package to compute the descriptive statistics of the variable `numVar` and use the function \color{blue} `hist()` \color{black} to visualize its histogram plot (try changing the argument \color{blue}`breaks`\color{black}).

5. Use the function \color{blue} `table()` \color{black} to create a frequency table of the variable `Q02` (i.e., "*On which weekday would you prefer to have the lectures?*")

6. Repeat point #4 but only consider the students that replied "Sì" (yes) to the question `Q01` (i.e., "*Do you plan to attend all lectures?*")

7. Cross the frequency of the variables `Q02` and `Q03` (i.e., "*Will you participate even if the lectures will be on a different day than the one you prefer?*") using \color{blue} `table()`
...
par(mfrow = c(2,2), mai = c(0.5,1,0.2,0.5)) # setting graphical parameters
...
vioplot::vioplot(extra ~ group, data = sleep) # violin plot: density by group
``` 
...
1. Download and read the file `studqs.RData` (it's the same dataset used in the previous exercise)

2. Generate the histogram of the variable `numVar` separately for those that replied "Sì" (*yes*) to question `Q01` ("*Do you plan to attend all lectures?*") and those that replied "Non a tutti" (*not all lectures*); add a vertical red line corresponding to the mean value of `numVar` for the two groups

3. Generate a normal QQ plot of the variable `numVar`. Is it normally distributed?

4. Generate the box plot of the variable `numVar` by the variable  `Q01`: which group has the highest median? Which has the hugest variability?

5. Create a new variable `numVar2` corresponding to `numVar2 - 2 * rnorm(nrow(qs), sd = 20)`

6. Generate the scatter plot of `numVar2` vs `numVar`, add the regression line with the \color{blue}`abline()`\color{black} function, and compute the correlation coefficient between the two variables
...
- Zandonella, C., Gambarota, F. (2021). Introduction to R (in Italian). Corso per imparare le basi di R

## Other resources/books introducing R
